...
# Вывод публичных и приватных ключей
print('1) Создание публичных и приватных ключей абонентом 1 и 2:')
print(getKeys("Alice", pubA, privA))
print(getKeys("Bob", pubB, privB))

# Создание сообщения со стороны Алисы
mAlice = 333
print("Исходное сообщение от 1го абонента: ", mAlice)

# Создание цифровой подписи со стороны Алисы
sAlice = encryptDecrypt(mAlice, privA)
print("2) Создание цифровой подписи со стороны абонента 1: ", sAlice)

# Передача публичного ключа от Боба Алисе
print("3) Передача публичного ключа от абонента 2, абоненту 1")
pubB = [pubB[0], pubB[1]]

# Шифрование сообщения и подписи публичным ключом Боба
print("4) Шифрование сообщения и подписи публичным ключом 2го абонента:")
CmAlice = encryptDecrypt(mAlice, pubB)
CsAlice = encryptDecrypt(sAlice, pubB)
print("Шифр сообщения: ", CmAlice)
print("Шифр подписи: ", CsAlice)

# Передача зашифрованного сообщения и подписи Бобу
print('5) Передача зашифрованного сообщения и подписи 2му абоненту')
CsmAlice = [CmAlice, CsAlice]

# Расшифрование сообщения и подписи Алисы
print('6) Расшифрование сообщения и подписи 1го абонента:')
DmBob = encryptDecrypt(CmAlice, privB)
DsBob = encryptDecrypt(CsAlice, privB)
print("Дешифр сообщения: ", DmBob)
print("Дешифр подписи: ", DsBob)

# Создание прообраза сообщения публичным ключом Алисы
print("Создание прообраза сообщения публичным ключом 1го абонента")
_mAlice = encryptDecrypt(DsBob, pubA)
print(" m = %s : _m = %s" % (DmBob, _mAlice))

# Проверка цифровой подписи
print('\nРезультат проверки:')
if _mAlice == DmBob: print("Цифровая подпись проверена. Сигнатура верна.")
else:
    print("Внимание! Сигнатура исходного сообщения отличается от вашего. Сообщение было изменено!")
# '''